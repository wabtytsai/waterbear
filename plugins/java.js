 ...
 *    JAVA PLUGIN
 * 
 *    Support for writing Java using Waterbear
 *
 */

(function(){
	
	//Disable waterbear features we aren't using:
	$('.run_scripts, .runScripts, .goto_stage, .stage').hide();

	// Pre-load dependencies (js and css)
	yepnope({
	...
	// == Load template ==
	//This is useful for specialized implemtations that rely on code that need not be exposed to the user
	var CodeTemplate = {
		template: "//depend// \n\n //main// \n\n //end//",	//template sections are enclosed by //section//
		positions: ['depend', 'main', 'end'],	//Locations in the template to look for
		defaultPos: 'main'	//This must be one of the positions defined on the array above, or no code will be generated
	};
	
	//Holds the generated script to later be puttoghether into the template.
	//Filled automatically. Leave Blank!
	var sections = {};
	
	//Set the various positions to "" so that when we appent values later we don't have to worry about undefined
	for(var i in CodeTemplate.positions) {
		sections[CodeTemplate.positions[i]] = "";
	}
	
	/* ====================================================================== */
	/**function to determine where the code generator should keep processing
	 *	@return: string | script to process
	 *	@note: called by Block.prototype.code in blocks.js:~461
	 */
	
	window.getMain = function(scriptobj) {
		var out = "";	//We have to do something with each part of the scriptobj, so return at the end
		for(var key in scriptobj) {	//Where key is a position name, and scriptobj[key] is the script associated with that key 
			if(key == CodeTemplate.defaultPos) {
				out = scriptobj[key];
			}
			else{
			//Need to handle this case
			//this is not the default pos code, we need to add it to its spot in the template
			sections[key] = scriptobj[key];
			}
		}
		return out;
	}
	...
	jQuery.fn.extend({
		/* ====================================================================== */
		/**Called to format the script
		 *	@return string | nicely formatted script
		 *	@note: called by writeScript below
		 */
		prettyScript: function(){
			
			//what was the point of this map? It was calling scructured script twice.
			//return js_beautify(this.map(function(){ 
			//	return $(this).structured_script();//$(this).extract_script();
			//}).get().join(''));
			
			return js_beautify($(this).structured_script());
		},
		
		/* ====================================================================== */
		/**Called when the script text is need
		 *	@param jQuery | where the code should be appended
		 *	@note: called from ui.js:~26
		 */
		writeScript: function(view){
			view.html('<pre class="language-java">' + $(this).prettyScript() + '</pre>');
		//hljs.highlightBlock(view.children()[0]);
		},
		
		/* ====================================================================== */
		/** Called when attempting to run the script.
		 * @use: see the javascript plugin for a use case
		 * @note: called from ui.js:~36
		 */
		wrapScript: function(){
		},
		
		/* ====================================================================== */
		/** If the block's position attribute matches the parameter, generate script for that block
		 * @param string | position to match block to
		 * @note: Used to generate script for positions. See structured_script below
		 */
		extract_script_filtered: function(position){
			//console.log('extract_script this', this.data());
			
			var self = this.data('model');
			if(typeof self.script == "object")
			{
				for(var item in self.script)
				{
					//item is position
					//self.script[item] is code to be evaled
			...
			//Default the position to the specified value in the Codetemplate
			if(this.data('position') == undefined)
				this.data('position', CodeTemplate.defaultPos)

			//If the blocks position matches the position we are looking for, generate the code
			if(this.data('position') == position)
			{
				return self.code();	//formely extract script
			}
			return '';
		},
		 ...
		 * @Call_chain: writeScript -> prettyScript -> structured_script
		 * @note: Blocks that generate more than one portion of script, will only 
		 * have the default position processed for locals, labels or next areas
		 */
		structured_script: function(){
			console.trace();
			var positions = CodeTemplate.positions;	//Positions are locations in the template.
			
			var self = this;	//html block
			
			//Sections are containers that will hold the generated script before it is inserted into the template
			//var sections = {};	//moving to a higher scope for the getMain function
			
			//extract script from the blocks for each position and stow that in the corresponding section
			for(var k in positions) {
				sections[positions[k]] += $(this).extract_script_filtered(positions[k]);
			}
			
			var structured = CodeTemplate.template;	//structured holds the template
			
			//Put the generated code into the template.
			$.each(sections, function(index, section){
				structured = structured.replace("//"+index+"//", section);
			});
			
			//return structured code. (may not be formatted very nicely)
			return structured;
		}
	});
...
/* ====================================================================== */
/** Choice Lists
 * Choice lists are used by blocks for drop down menus
 * @note: expose these globally so the Block/Label methods can find them
 * @fixme: if more than one plugin has choice lists the last plugin will override the others
 */
	window.choiceLists = {
